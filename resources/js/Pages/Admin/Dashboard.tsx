...
import { Users, GraduationCap, School, FileText, CheckCircle, TrendingUp, Tag, ClipboardList, Bot, AlertTriangle } from 'lucide-react'
    ...
    return (
        <AppLayout title="Admin Dashboard" subtitle="Pantau & kelola seluruh platform QuizQuest">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-8">
                {[
                    { icon: <Users className="w-6 h-6" />, label: 'Total User', value: stats.total_users, color: 'violet' as const, href: '/admin/users' },
                    { icon: <GraduationCap className="w-6 h-6" />, label: 'Murid', value: stats.total_students, color: 'blue' as const, href: '/admin/users' },
                    { icon: <School className="w-6 h-6" />, label: 'Guru', value: stats.total_teachers, color: 'emerald' as const, href: '/admin/users' },
                    { icon: <FileText className="w-6 h-6" />, label: 'Total Quiz', value: stats.total_quizzes, color: 'amber' as const, href: '/admin/quizzes' },
                    { icon: <CheckCircle className="w-6 h-6" />, label: 'Quiz Aktif', value: stats.published_quizzes, color: 'emerald' as const, href: '/admin/quizzes' },
                ].map((s, i) => (
                    <StatCard key={s.label} index={i} icon={s.icon} label={s.label} value={s.value.toLocaleString()} color={s.color} href={s.href} />
                ))}
            </div>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                        ...
                        <p className={`text-sm ${aiEnabled ? 'text-emerald-700 dark:text-emerald-300' : 'text-amber-700 dark:text-amber-300'}`}>
                            {aiEnabled ? 'AI Assistant berjalan dengan OpenRouter API' : 'Konfigurasi OpenRouter API Key di pengaturan untuk mengaktifkan AI'}
                        </p>
                    </div>
