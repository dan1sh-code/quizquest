...
export default function StatCard({ icon, label, value, sub, color = 'violet', index = 0, href }: StatCardProps) {
    const InnerContent = (
    ...
    return (
        <motion.div
            ...
            className={cn(
                "bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm card-hover overflow-hidden",
                href ? "hover:border-violet-500/50 hover:shadow-md transition-all cursor-pointer" : ""
            )}
        >
            {href ? (
