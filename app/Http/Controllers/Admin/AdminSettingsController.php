...
class AdminSettingsController extends Controller {
    ...
    public function update(Request $r) {
        foreach(['site_name','site_tagline','groq_model','quiz_xp_base','quiz_xp_perfect','quiz_xp_streak'] as $k) if($r->has($k)) Setting::set($k,$r->input($k));
        foreach(['ai_enabled','allow_registration','maintenance_mode'] as $k) Setting::set($k,$r->has($k)?'1':'0');
        if($r->filled('groq_api_key')) Setting::set('groq_api_key',$r->groq_api_key);
